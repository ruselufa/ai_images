...
type SubscriptionService struct {
	...
	bot    interface {
		...
		SendSubscriptionRenewedMessage(userID int64) error
...
func (s *SubscriptionService) ProcessRecurringPayment(subscription *domain.Subscription) error {
	...
	if status == "succeeded" {
		...
		s.sendSubscriptionRenewedMessage(subscription.UserID)
...
// sendSubscriptionRenewedMessage отправляет уведомление о продлении подписки
func (s *SubscriptionService) sendSubscriptionRenewedMessage(userID int64) {
	if s.bot != nil {
		if err := s.bot.SendSubscriptionRenewedMessage(userID); err != nil {
			log.Printf("❌ Failed to send subscription renewed message to user %d: %v", userID, err)
		} else {
			log.Printf("📨 Subscription renewed message sent to user %d", userID)
		}
	} else {
		log.Printf("📨 Should send subscription renewed message to user %d - bot not configured", userID)
	}
}
