...
// SendSubscriptionRenewedMessage отправляет сообщение о продлении подписки
func (h *SubscriptionHandler) SendSubscriptionRenewedMessage(userID int64) error {
	if h.bot == nil {
		log.Printf("�� [BOT] Cannot send message - bot not set for user %d", userID)
		return fmt.Errorf("bot not set")
	}

	// Получаем информацию о подписке для отображения даты завершения
	subscription, err := h.subscriptionService.GetUserSubscription(userID)
	if err != nil {
		log.Printf("❌ [BOT] Error getting subscription for user %d: %v", userID, err)
		// Отправляем сообщение без даты, если не удалось получить информацию
		subscription = nil
	}

	var messageText string
	if subscription != nil {
		// Форматируем дату завершения подписки
		endDate := subscription.NextPayment.Format("02.01.2006")
		endTime := subscription.NextPayment.Format("15:04")

		messageText = "✅ *Подписка продлена!*\n\n" +
			"Ваша подписка была успешно продлена.\n\n" +
			"*Дата завершения подписки:* " + endDate + " в " + endTime + "\n\n" +
			"Спасибо за использование SocialFlow! 🚀\n\n" +
			"Теперь вы можете создавать неограниченное количество контента."
	} else {
		// Fallback сообщение без даты
		messageText = "✅ *Подписка продлена!*\n\n" +
			"Ваша подписка была успешно продлена.\n\n" +
			"Спасибо за использование SocialFlow! 🚀\n\n" +
			"Теперь вы можете создавать неограниченное количество контента."
	...
	_, err = h.bot.Send(msg)
	if err != nil {
		log.Printf("❌ [BOT] Failed to send subscription renewed message to user %d: %v", userID, err)
		return err
	}

	log.Printf("📨 [BOT] Subscription renewed message sent to user %d", userID)
	return nil
}
